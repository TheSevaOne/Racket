...
(struct area (snake apple pause?) #:transparent)
;ВСЕ ВМЕСТЕ
...
(define HEAD-IMG (triangle 10 "solid" "red"))
...
(define (next w)
  ...
  (if (area-pause? w)
      w
      (if apple-to-eat
          (area (grow snake) (age-apple (eat apples apple-to-eat)) #f);;area(snake apple score +1)
          (area (slither snake) (age-apple apples) #f))))
...
(define (gamemode d w)
  ( cond
     [(key=? d "1")
      (area (snake  "down" (list (posn  1 4) #f))
            (list (fresh-apple)
                  (fresh-apple)
                  (fresh-apple)
                  (fresh-apple)))]
     [(key=? d "2") (area (snake  "down" (list (posn  1 4))) (list (fresh-apple)) #f)]
     [(key=? d "p") (struct-copy area w [pause? #t])]
     [(key=? d "q") (exit)] 
     [else w]))
...
(define (tofile  file  score)
  (write-to-file  score file   #:exists  'truncate
                  #:mode 'text))
...
(define (renew apple)
  (cond [(empty? apple) empty]
        [(done? (first apple))(cons (fresh-apple) (renew (rest apple)))]
        [else (cons (first apple) (renew (rest apple)))]))
